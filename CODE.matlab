>> [tel,fs] = audioread('tel.wav'); %reads the >>%data from the tel.wav file as the vector tel >>%at sampling frequency fs
...
figure;
plot(f, abs(TEL1)); %plotting the magnitude of the spectrum xlabel(’Frequency - Hz’);
